...
class ReportFaceColorScreen(Frame):
    def __init__(self, master):
        ...
        def re_start_FC(event):
            ...
            self.report = Label(self,background='#D9D9D9', text=label_text, compound=TOP, padx=10, font='Arial 12', anchor=N)
            self.report.place(x=136,y=103, width=368, height=291)
            # Label 위젯에 PIL 이미지를 표시
            self.report = Label(self, image=self.image_tk1, compound=TOP, padx=10)
            self.report.place(x=166, y=216, width=132, height=93)
            self.report = Label(self, image=self.image_tk2, compound=TOP, padx=10)
